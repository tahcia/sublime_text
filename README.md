...
* **`uploadOnSave`** *(Integer)*: Controls automated background syncing.
* `1` (or any non-zero integer): **Enabled**. Automatically uploads `.tahcia.json` files to the cloud when saved.
* `0`: **Disabled**. Files will only upload when manually triggered via the context menu or command palette.
